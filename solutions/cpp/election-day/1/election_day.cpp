...
void increment_vote_count(ElectionResult& election_result, int count){
    election_result.votes += count;
}
...
{
    int max = 0;
    ElectionResult* result = &candidates[0];
    for(ElectionResult& elec : candidates)
    {
        if(elec.votes > max)
        {
            max = elec.votes;
