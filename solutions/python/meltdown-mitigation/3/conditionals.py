...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    # we dont use range() since that actually creates the object with all 
    # those numbers and then we do an inclusion check which worst case O(n)
    # so instead we do bounds checks which are gauranteed O(1)
