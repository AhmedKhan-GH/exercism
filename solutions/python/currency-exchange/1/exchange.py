...
def exchangeable_value(budget, exchange_rate, spread, denomination):
    ...
    # what am I trying to calculate, the max value of the new currency
    # in denominations (not remainder) after exchange rate and spread
    
    true_rate = exchange_rate * (1 + spread / 100)

    new_currency = budget / true_rate # total budget in new currency

    return (new_currency // denomination) * denomination

    # the maximum value I can get with the new currency is taking whats not the remainder (would I calculate that with a single operation or a subtraction?) and then multiplying it by the exchange rate, I also havee to consider the spread 
